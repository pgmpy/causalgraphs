...
impl Independencies {
    ...
    pub fn closure(&self) -> Self {
        ...
        let mut new_inds: HashSet<IndependenceAssertion> =
            self.assertions.iter().cloned().collect();
        ...
        while !new_inds.is_empty() {
            ...
            let mut new_pairs: HashSet<(&IndependenceAssertion, &IndependenceAssertion)> =
                HashSet::new();
            let all_current: Vec<IndependenceAssertion> =
                all_independencies.iter().cloned().collect();
            ...
            next_round.retain(|ind| {
                !ind.event1.is_empty()
                    && !ind.event2.is_empty()
                    && !all_independencies.contains(ind)
            });
            new_inds = next_round;
        }
    ...
    fn sg2_weak_union(&self, ind: &IndependenceAssertion) -> Vec<IndependenceAssertion> {
        ...
        if ind.event2.len() > 1 {
            for elem in &ind.event2 {
                ...
                if let Ok(assertion) =
                    IndependenceAssertion::new(ind.event1.clone(), new_event2, Some(new_event3))
                {
                    results.push(assertion);
                }
            }
        ...
        if ind.event1.len() > 1 {
            for elem in &ind.event1 {
                ...
                if let Ok(assertion) =
                    IndependenceAssertion::new(new_event1, ind.event2.clone(), Some(new_event3))
                {
                    results.push(assertion);
                }
            }
    ...
    fn sg3_contraction(
        &self,
        ind1: &IndependenceAssertion,
        ind2: &IndependenceAssertion,
    ) -> Vec<IndependenceAssertion> {
        let mut results = Vec::new();
    ...
    fn try_contraction_all_sym(
        &self,
        larger: &IndependenceAssertion,
        smaller: &IndependenceAssertion,
    ) -> Vec<IndependenceAssertion> {
        let mut res = Vec::new();
        if let Some(contracted) = self.try_contraction(larger, smaller) {
            res.push(contracted);
        }
        if let Some(contracted) = self.try_contraction(smaller, larger) {
            // Try both orders
            res.push(contracted);
        }
        res
    ...
    fn try_contraction(
        ...
        if y.is_subset(y_z) && y != y_z && z.is_subset(y_z) && z != y_z && y.is_disjoint(z) {
    ...
    pub fn reduce(&self) -> Self {
        let mut unique_assertions: Vec<IndependenceAssertion> = self
            .assertions
            .iter()
            .cloned()
            .collect::<HashSet<_>>()
        ...
        for assertion in unique_assertions {
            let temp_independencies: Independencies =
                Self::from_assertions(reduced_assertions.clone());
    ...
    pub fn entails(&self, other: &Independencies) -> bool {
        ...
        other
            .assertions
            .iter()
            .all(|assertion| closure_assertions.contains(assertion))
    }
